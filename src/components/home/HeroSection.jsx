...
export default function HeroSection({ walletAddress, setWalletAddress, handleAnalyze, isAnalyzing }) {
    ...
    <section className="relative w-full min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-[1400px] mx-auto px-4 sm:px-6 text-center py-20">
        ...
        <motion.p
          ...
          className="text-base sm:text-lg md:text-xl lg:text-2xl text-white/80 mb-8 sm:mb-12 font-light"
        >
        ...
        >
          <div className="relative">
            <div className="flex flex-col sm:flex-row gap-3">
              <Input
                ...
                className="h-12 sm:h-14 md:h-16 text-sm sm:text-base md:text-lg bg-white/5 border-white/20 text-white placeholder:text-white/40 focus:border-[#FF5C1A] transition-all"
              ...
              <BeveledButton
                ...
                className="h-12 sm:h-14 md:h-16 px-6 sm:px-8 text-sm sm:text-base md:text-lg whitespace-nowrap"
                style={{ fontFamily: 'TECHNOS, sans-serif' }}
              >
                {isAnalyzing ? (
                  <>
          ...
          <p className="text-xs sm:text-sm text-white/40 mt-3 sm:mt-4">
          ...
          >
            <div className="flex items-center justify-center gap-2 mb-4">
              <Crown className="w-5 h-5 text-[#FF5C1A]" />
              <p className="text-sm sm:text-base text-white/60 uppercase tracking-wider">
            ...
            <div className="grid grid-cols-3 gap-3 sm:gap-4">
              {topLoosers.map((looser) => (
                <div
                  ...
                  className="bg-white/5 border border-white/10 p-3 sm:p-4 hover:bg-white/10 hover:border-[#FF5C1A]/50 transition-all cursor-pointer group"
                >
                  <div className="flex items-center justify-center gap-1 sm:gap-2 mb-2">
                    <Crown className={`w-3 h-3 sm:w-4 sm:h-4 ${getRankColor(looser.rank)} group-hover:scale-110 transition-transform`} />
                    <span className={`text-lg sm:text-xl font-bold ${getRankColor(looser.rank)}`}>
                      #{looser.rank}
                    </span>
                  </div>
                  <p className="text-[10px] sm:text-xs text-white/60 font-mono mb-2">
                    {formatWallet(looser.wallet)}
                  </p>
                  <p className="text-sm sm:text-base font-bold text-[#FF223B] group-hover:scale-105 transition-transform">
