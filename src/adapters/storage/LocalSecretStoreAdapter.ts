...
import {
  ...
  Tag,
  SecretMetadata,
} from '@/interfaces/storage/ISecretStore';
...
export class LocalSecretStoreAdapter implements ISecretStore {
  ...
  constructor(storageDir?: string) {
    ...
    this.storageDir =
      storageDir || path.join(process.cwd(), '.local-storage', 'secrets');
  }
  ...
  async addSecretTags(secretName: string, tags: Tag[]): Promise<void> {
    ...
    for (const newTag of tags) {
      const existingIndex = secret.tags.findIndex(
        (t) => t.Key === newTag.Key
      );
      if (existingIndex >= 0) {
