      ...
      <div className="max-w-[1400px] mx-auto w-full px-4 sm:px-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8">
          {stats.map((stat, index) => (
            ...
            >
              <div className="bg-white/5 border border-white/10 p-6 sm:p-8 hover:bg-white/10 hover:border-[#FF5C1A]/50 transition-all duration-300 h-full" style={{ borderRadius: 0 }}>
                <div className="w-14 h-14 sm:w-16 sm:h-16 mx-auto mb-4 sm:mb-6 bg-gradient-to-r from-[#FF5C1A] via-[#FF223B] to-[#30080C] flex items-center justify-center group-hover:scale-110 transition-transform duration-300" style={{ clipPath: 'polygon(0 0, 100% 0, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0 100%)' }}>
                  <stat.icon className="w-7 h-7 sm:w-8 sm:h-8 text-white" />
                </div>

                <div 
                  className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-2 sm:mb-3 text-center"
                  style={{ fontFamily: 'TECHNOS, sans-serif' }}
                >
                  <CountUp end={stat.value} format={stat.format} />
                </div>

                <p className="text-white/60 text-center text-xs sm:text-sm uppercase tracking-wider">
                  {stat.label}
                </p>
              </div>
            </motion.div>
        ...
        >
          <p className="text-white/40 text-base sm:text-lg italic">
