...
export class StorageFactory {
  ...
  private static detectProvider(): StorageProvider {
    ...
    const hasAwsProfile =
      process.env.AWS_DEFAULT_PROFILE || process.env.PROFILE;

    if (isDevelopment && !hasAwsProfile) {
      console.log(
        '[StorageFactory] Auto-detected: Using local storage (no AWS profile found)'
      );
      return 'local';
    }
  ...
  static createParameterStore(config?: StorageConfig): IParameterStore {
    ...
    switch (provider) {
      ...
      case 'local':
        return new LocalParameterStoreAdapter(
          config?.localStorageDir,
          config?.parameterStorePrefix
        );
